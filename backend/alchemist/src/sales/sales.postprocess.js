...
const buildFallbackBundle = ({ recommendedProducts = [], detectedIntent }) => {
  ...
  return {
    title: detectedIntent === 'objection' ? 'Opcion clara para avanzar' : 'Propuesta sugerida',
    summary: `Una combinacion concreta con ${recommendedProducts.map((item) => item.name).join(', ')} pensada para seguir avanzando con una propuesta clara.`,
